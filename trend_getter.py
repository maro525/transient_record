...
class TrendGetter:
    def __init__(self):
        ...
        self.trend_words = self.get_words()
        # self.trend_words = ['レコードたい', 'にいがた', 'みずのみき', 'たにぐちゆきひろ', 'ポケモン', 'アナとゆきの', 'やまのてせん', 'ワンパチ', 'いながきけいた', 'いきなりステーキ', 'ハンセンびょう', 'おかむらたかし', 'あさひなお', 'ブラックフライデー', 'りんごちゃん', 'エリカさま']
        # self.trend_words = ['エリカさま']
        self.demo_index = 0
    ...
    def get_random_word_demo(self):
        ...
        self.demo_index += 1
        if self.demo_index >= len(self.trend_words):
            self.demo_index = 0
        return tw
